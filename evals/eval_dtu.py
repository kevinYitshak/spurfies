...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir_root", type=str, default="data_s_volsdf", help="GT data dir"
    )
    ...
    for scan in scans:
        ...
        pred_datapath = f"results/REPRODUCE/mesh/final/{args.scan:03d}.ply"
        ...
        stl_pcd = o3d.io.read_point_cloud(
            f"{dataset_dir}/Points/stl/stl{args.scan:03}_total.ply"
        )
        ...
        try:
            ...
            if args.scan == 82:
                ...
                ground_plane = loadmat(f"{dataset_dir}/ObsMask/Plane{args.scan}.mat")[
                    "P"
                ]
        except:
            pbar.close()
            mean_d2s, mean_s2d, over_all = 10000.0, 10000.0, 10000.0
            print(
                "scan{:0>3} {:.2f} {:.2f} {:.2f}".format(
                    args.scan,
                    trun_n_d(mean_d2s, 2),
                    trun_n_d(mean_s2d, 2),
                    trun_n_d(over_all, 2),
        ...
        if False:   # visualization
            ...
            data_color[np.where(inbound)[0][grid_inbound][in_obs]] = (
                R * data_alpha + W * (1 - data_alpha)
            )
            data_color[
                np.where(inbound)[0][grid_inbound][in_obs][dist_d2s[:, 0] >= max_dist]
            ] = G
            write_vis_pcd(
                f"{vis_out_dir}/vis_{args.scan:03}_d2s.ply", data_down, data_color
            )
