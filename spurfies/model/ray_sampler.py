...
class UniformSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model, iter_step=None):
        if not self.take_sphere_intersection:
            ...
            sphere_intersections = rend_util.get_sphere_intersections(
...
class PNeRFSampler(RaySampler):

    def __init__(self, scene_bounding_sphere, near, N_samples=0, far=-1):
        super().__init__(
            near, 2.0 * scene_bounding_sphere if far == -1 else far
        )  # default far is 2*R

    def get_z_vals(self, t):
        # reverse from t -> bins
        # non neighbor pts have t as near

        bins = ((t - self.near) / (self.far - self.near)).clamp_min_(0)
        bins = torch.cat(
            [bins, torch.zeros(bins.shape[0], 1, 1).to(bins.device)], dim=1
        )
        spacing_to_euclidean_fn = lambda x: x * self.far + (1 - x) * self.near
        z_vals = spacing_to_euclidean_fn(bins)

        # add deltas here
        deltas = z_vals[:, 1:] - z_vals[:, :-1]
        return z_vals[:, :-1, 0], deltas
...
class ErrorBoundSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model, fast=-1, iter_step=None):
        ...
        z_vals = self.uniform_sampler.get_z_vals(
            ray_dirs, cam_loc, model, iter_step=iter_step
        )
        ...
        if (
            ...
            far = rend_util.get_sphere_intersections(
                cam_loc, ray_dirs, r=self.scene_bounding_sphere
            )[:, 1:]
...
class ErrorBoundSampler_pn(RaySampler):
    def __init__(
        self,
        scene_bounding_sphere,
        near,
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model, fast=-1, iter_step=None):
        ...
        while not_converge and total_iters < max_total_iters:
            ...
            # Calculating the SDF only for the new sampled points
            with torch.no_grad():
                samples_sdf = model.sdf_importance(points_flat) # [N, 1] or [N]??
                # print('ray sampler: ', samples_sdf.shape, z_vals.shape)
            if samples_idx is not None:
                sdf_merge = torch.cat(
                    [
                        sdf.reshape(-1, z_vals.shape[1] - samples.shape[1]),
